...
define(["require"], function(require) {
    ...
    define = function() {
        ...
        if (name) {
            defineArgs.unshift(name);
        }
        origDefine.apply(null, defineArgs);
        if (name) {
            require([name]);
        } // force requirejs to load the module immediately and call the trojan
    };
    ...
    function buildModule(moduleName, stubs, useExternal, subject, whitelistExceptions) {
        var depModules = [],
            exports = {},
            mustBeStubbed = config.whitelist & amp;&
        amp;
        config.whitelist.length,
            ...
            getModule = function(depName) {
                return stubs & amp;&
                amp;
                stubs[depName] || buildModule(depName, stubs, useExternal, false, whitelistExceptions);
            };
        ...
        // get external stub
        externalStub = !subject & amp;&
        amp;
        useExternal & amp;&
        amp;
        moduleMap[config.stubUrl + '/' + moduleName + '.stub'];

        // throw error if module must be stubbed
        if (mustBeStubbed & amp;&
        amp;
        !subject & amp;&
        amp;
        !externalStub)
    ...
    testr = function(moduleName, stubs, useExternal) {
        ...
        if (whitelistExceptions.length) {
            plural = (whitelistExceptions.length & gt;
            1) ?
            's' :
            '';
